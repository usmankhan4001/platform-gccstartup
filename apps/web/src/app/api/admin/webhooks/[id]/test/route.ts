...
/**
 * Fires a real signed test POST to the endpoint. The delivery row is written
 * FIRST with its payload, so a crash or a hanging endpoint still leaves an
 * auditable record; the outcome is then patched in. A network failure is
 * recorded as a failed delivery, never raised as a 500.
 */
